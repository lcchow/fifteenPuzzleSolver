...
//        System.out.println(targetBoard.toString());
    ...
    public static Board getSmallTargetBoard(int size) throws IOException {
        int newSize = 5;
        int sizeDiff = size - newSize;
        int[][] target = new int[newSize][newSize];
        int k = 1;
        ...
        Board targetBoard = new Board(target,newSize);
        System.out.println("SMLTARGET:");
        System.out.println(targetBoard.toString());
        return targetBoard;
    }

    public static <T> Vertex<T> solvePuzzle(File input, File output) throws IOException {
        Board startBoard = new Board(input);
        Vertex<T> start;
        Board targetBoard;


        if (startBoard.size > 5) {
            int diff = startBoard.size - 5;
            Board tmpBoard = solveBigPuzzle(startBoard,output);

            int[][] newStartBoard = new int[5][5];
            int ind1 = 0;
            int ind2 = 0;

            for (int i = diff; i<tmpBoard.size;i++) {
                for (int j = diff; j<tmpBoard.size;j++) {
                    newStartBoard[ind1][ind2] = tmpBoard.board[i][j];
                    ind2++;
                }
                ind2 = 0;
                ind1++;
            ...
            Board smallStartBoard = new Board(newStartBoard,5);
            System.out.println(smallStartBoard.toString());

            start = new Vertex<T>(smallStartBoard);

            targetBoard = getSmallTargetBoard(startBoard.size);
            System.out.println("EXIT BIG PUZZLE");

        } else {
            start = new Vertex<T>(startBoard);
            targetBoard = getTargetBoard(input);
        }
...
//        System.out.println(simpleBoard.toString());
    ...
    public static <T> Board solveBigPuzzle(Board input, File output) throws IOException {
...
//
//        for (int i = 0; i<targetBoard.size; i++) {result
//            for (int j = 0; j < targetBoard.size; j++) {
//                targetMap.put(targetBoard.board[i][j], new ArrayList<Integer>());
//                targetMap.get(targetBoard.board[i][j]).add(i);
//                targetMap.get(targetBoard.board[i][j]).add(j);
...
//        System.out.println("SIZE DIFF: " +sizeDiff);
        ...
        // FILE PRINT FOR BIG PUZZLE

        FileWriter fw = new FileWriter(output);

        while (result.size() > 0 ){
            String res = result.removeFirst();
...
//		}
		...
		// SOLVE BOARD
		Vertex<T> result = solvePuzzle(input,output);
